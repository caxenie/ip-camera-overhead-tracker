    ...
    while(key != 'q')
    {
        ...
        if(cluster->state==ON_TRACK)
        {
            ...
            for(int p=0; p<cluster[0].data->NUM_OF_MARKERS; ++p)
            {
                ...
                marker_position->cluster_coordinates[cluster->id]->net_coordinates[p].x = cluster->net_coordinates->x;
                marker_position->cluster_coordinates[cluster->id]->net_coordinates[p].y = cluster->net_coordinates->y;
                marker_position->cluster_coordinates[cluster->id]->net_coordinates[p].z = cluster->net_coordinates->z;
...
void compute_net_coordinates(StereoCluster *cluster)
{
    ...
    CvPoint2D32f marker_pxl_coord[2];
    CvPoint3D32f marker_net_coord;

    for(int nm=0; nm<cluster->data->NUM_OF_MARKERS; ++nm)
    {
        /* read out and normalize image pixel coordinates between -1 and 1 */
        marker_pxl_coord[0].x = 2.0*(cluster->tracker[0]->marker[nm]->pos_measured.x/(double)FRAME_WIDTH) - 1.0;
        marker_pxl_coord[0].y = 2.0*(cluster->tracker[0]->marker[nm]->pos_measured.y/(double)FRAME_HEIGHT) - 1.0;
        marker_pxl_coord[1].x = 2.0*(cluster->tracker[1]->marker[nm]->pos_measured.x/(double)FRAME_WIDTH) - 1.0;
        marker_pxl_coord[1].y = 2.0*(cluster->tracker[0]->marker[nm]->pos_measured.y/(double)FRAME_HEIGHT) - 1.0;

        compute_rotated_net_output(cluster, marker_pxl_coord, &marker_net_coord);

        cluster->net_coordinates[nm].x = marker_net_coord.x;
        cluster->net_coordinates[nm].y = marker_net_coord.y;
        cluster->net_coordinates[nm].z = marker_net_coord.z;
...
void *reproject(void *arg)
{
    ...
    **/
    ...
    CvPoint3D32f *r_init = (CvPoint3D32f *) calloc(data->NUM_OF_MARKERS,sizeof(CvPoint3D32f));
...
//      printf("%d stereo clusters on track\n");
        ...
        CvPoint3D32f *net = (CvPoint3D32f *)calloc(k,sizeof(CvPoint3D32f));
        ...
        for(int nm=0; nm<data->NUM_OF_MARKERS; ++nm)
        {
            for(int p=0; p<k; ++p)
            {
                net[nm].x += marker_position->cluster_coordinates[p]->net_coordinates[nm].x;
                net[nm].y += marker_position->cluster_coordinates[p]->net_coordinates[nm].y;
                net[nm].z += marker_position->cluster_coordinates[p]->net_coordinates[nm].z;
            }
        }
        ...
        for(int nm=0; nm<data->NUM_OF_MARKERS; ++nm)
        {
            ...
            if(!init[nm])
            {
                r_init[nm].x = net[nm].x;
                r_init[nm].y = net[nm].y;
                r_init[nm].z = net[nm].z;
                init[nm] = 1;
            }
            ...
            measurement(0) = net[nm].x - r_init[nm].x;
            measurement(1) = net[nm].y - r_init[nm].y;
            measurement(2) = net[nm].z - r_init[nm].z;
